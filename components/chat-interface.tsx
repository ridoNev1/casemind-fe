...
const ChatInterface = () => {
  ...
  const handleSubmit = (message?: PromptInputMessage) => {
    const hasText =
      ...
      (message as any).text.trim().length > 0;

    const files =
      typeof message === "object" && message !== null && "files" in message
        ? (message as any).files
        : undefined;
    const hasAttachments =
      !!files &&
      typeof (files as { length?: number }).length === "number" &&
      (files as { length: number }).length > 0;
    ...
    if (hasAttachments && files) {
      toast.success("Files attached", {
        description: `${
          (files as { length: number }).length
        } file(s) attached to message`,
      });
    }

    const textToSend =
      hasText && typeof (message as any).text === "string"
        ? (message as any).text
  ...
  return (
    <div className="relative flex size-full max-h-screen flex-col divide-y overflow-hidden">
      <Conversation>
        <ConversationContent>
